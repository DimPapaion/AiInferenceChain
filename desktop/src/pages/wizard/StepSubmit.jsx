...
export default function StepSubmit({
  ...
  const submit = async () => {
    ...
    try {
      ...
      const data = await resp.json();
      if (!data.ok) {
        setError(data.error || JSON.stringify(data.response));
        setPhase('error');
        return;
      }
      setPhase('done');
  ...
  return (
    <div className="wstep-card">
      ...
      <div className={`submit-chain-status ${chainOnline === null ? 'checking' : chainOnline ? 'online' : 'offline'}`}>
        ...
        {chainOnline === false && 'Chain offline - you can still sign and save the manifest locally, then submit later.'}
      </div>
          ...
          {(() => {
            ...
            return (
              <>
                ...
                {last.val_acc != null && (
                  <Pill label="Val acc" value={`${(last.val_acc * 100).toFixed(1)}%`} accent />
                )}
      ...
      {phase !== 'done' && phase !== 'saved' && (
        <div className="wstep-field" style={{ marginTop: 16 }}>
          ...
          />
          <span className="hint">Your 32-byte Ed25519 private key in hex. It never leaves this device.</span>
        </div>
      )}

      {phase === 'idle' && <button className="btn-primary" onClick={sign}>Sign Manifest</button>}
      {phase === 'signing' && <div className="wstep-status info">Signing...</div>}
      ...
      {phase === 'signed' && (
        <>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 16, flexWrap: 'wrap' }}>
            ...
            {chainOnline ? (
              <button className="btn-primary" onClick={submit}>Submit to Network</button>
            ) : (
              <button className="btn-primary" onClick={saveLocally}>Save Locally (submit when online)</button>
            )}
          </div>
        </>
      )}
      ...
      {phase === 'submitting' && (
        <div className="wstep-status info">{chainOnline ? 'Submitting to network...' : 'Saving manifest locally...'}</div>
      )}
      ...
      {phase === 'saved' && (
        <>
          <div className="wstep-status ok" style={{ fontSize: 15, padding: '16px 20px' }}>
            ...
            Your public key: <code data-selectable style={{ fontSize: 11 }}>{pubKeyHex}</code><br />
            Open Dashboard > Node Status to submit when the network is live.
          </div>
          <div className="wstep-actions">
            <button className="btn-primary" onClick={onComplete}>Open Dashboard</button>
          </div>
